...
Additionally: module-import-time coverage wiring (parallel +
subprocess support). `os.environ.setdefault` would be a silent
no-op here because pytest-cov has already set ``COVERAGE_FILE``
to a tmp dir by the time conftest is loaded — so we force-set.
See ``scitex-dev`` skill leaf ``05_development_06_subprocess-coverage.md``.
"""
...
def _ensure_subprocess_coverage_shim() -> None:
    ...
    pth = purelib / "_scitex_introspect_subprocess_coverage.pth"
    shim = (
        "import os, coverage\n"
